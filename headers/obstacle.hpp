...
public:
    ...
    // Vérifier si l'obstacle bloque la ligne entre l'émetteur et le point cible
    ...
    bool isBlocking(double x, double y, double emitter_x, double emitter_y) const {
        ...
        if (std::abs(x1 - x2) < EPSILON) {
            return checkVerticalWall(x, y, emitter_x, emitter_y);
        }
        // Cas horizontal optimisé (avec épaisseur)
        else if (std::abs(y1 - y2) < EPSILON) {
            return checkHorizontalWall(x, y, emitter_x, emitter_y);
        }
        // Pour les obstacles rectangulaires généraux avec épaisseur
        else {
            return checkGeneralRectangle(x, y, emitter_x, emitter_y);
        }
    }

private:
    // Vérifier l'intersection avec un mur vertical
    bool checkVerticalWall(double x, double y, double emitter_x, double emitter_y) const {
        double wall_x = x1;
    ...
    // Vérifier l'intersection avec un mur horizontal
    bool checkHorizontalWall(double x, double y, double emitter_x, double emitter_y) const {
        double wall_y = y1;
    ...
    // Vérifier l'intersection avec un rectangle général (avec épaisseur)
    bool checkGeneralRectangle(double x, double y, double emitter_x, double emitter_y) const {
